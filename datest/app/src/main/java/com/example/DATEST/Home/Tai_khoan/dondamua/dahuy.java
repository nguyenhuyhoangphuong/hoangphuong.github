...
public class dahuy extends Fragment {
    ...
    private void getdata() {
        API.API.getadhllsp(idtk,"Đã hủy").enqueue(new Callback<List<donhang>>() {
            @Override
            public void onResponse(Call<List<donhang>> call, Response<List<donhang>> response) {
                if (response.isSuccessful()){
                    ...
                    sl=donhangs.size();
                    dondagiao_adapteter=new dondagiao_adapteter(donhangs,dahuy.this);
